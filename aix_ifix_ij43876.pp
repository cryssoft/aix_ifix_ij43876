#
#  2023/02/24 - cp - Updated with one more spin
#
#-------------------------------------------------------------------------------
#
#  From Advisory.asc:
...
#    7.2.5.2    IJ44114s2a.221102.epkg.Z  key_w_fix  perfstat
    ...
    if ($::facts['osfamily'] == 'AIX') {
        ...
        if (($ifixSuffix != 'unknown') and ($ifixBuildDate != 'unknown')) {
            ...
            unless ($ifixFullName in $::facts['aix_ifix']['hash'].keys) {
                ...
                $ifixStagingSource = "puppet:///modules/aix_ifix_ij43876/${ifixName}${ifixSuffix}.${ifixBuildDate}.epkg.Z"
